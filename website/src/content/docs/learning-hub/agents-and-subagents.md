...
Yes, since v1.0.72. **Multi-turn subagents are always enabled**, so you can send additional messages to a subagent that is already running — you don't have to wait for it to complete before refining or redirecting its work. This makes interactive supervision of long-running agentic tasks much more practical.
