...
The `/worktree` command (v1.0.61+) creates a new git worktree and switches into it, **leaving your uncommitted changes behind** in the original worktree. This lets you start a clean parallel branch without disturbing what you are currently working on:
...
> **Updated in v1.0.71**: `/worktree` and `/move` were split into two separate commands with distinct behaviors:
> - **`/worktree`** — creates a new worktree and **leaves** your uncommitted changes in the original worktree (clean context for the new branch)
> - **`/move`** — creates a new worktree and **carries** your uncommitted changes into it (the previous combined behavior)
>
> Use `/worktree` when you want to start fresh on a new branch, and `/move` when you want to continue your current in-progress work on a different branch.
...
> **Plan mode enforcement (v1.0.71+)**: Plan mode now **hard-blocks** built-in tool calls that would modify the workspace, so the agent can no longer edit files or run mutating shell commands while planning. Built-in mutators such as opening a pull request are also blocked. MCP and external tools are still allowed. This makes plan mode a true read-only planning phase — use it when you want the agent to propose an approach before making any changes.
