...
**Re-enabling a disabled server for a single run** (v1.0.79+): If you've disabled an MCP server in settings but want to use it for just one session, pass `--enable-mcp-server <name>` at startup. This overrides the disabled setting for the current run only — the server remains disabled for future sessions:
