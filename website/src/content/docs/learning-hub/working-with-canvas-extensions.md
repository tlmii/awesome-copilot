...
As of v1.0.71, **canvas support is also available in the GitHub Copilot CLI** for extension-driven interactions. Canvas extensions you build can be loaded and used from the CLI — the same surface can be updated by agents running in the app or from a terminal session.
