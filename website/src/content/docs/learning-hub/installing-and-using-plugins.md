...
*(v1.0.79+)* Add `"autoUpdate": true` to an `extraKnownMarketplaces` entry to **automatically update its plugins at every session start**:
...
With `autoUpdate` enabled, Copilot fetches the latest version of all installed plugins from that marketplace each time a session starts, so your team always has the most recent agents, skills, and hooks without needing to run `copilot plugin update` manually.
