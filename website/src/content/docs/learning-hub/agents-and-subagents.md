...
### Sending follow-up messages to running subagents

As of v1.0.71, **multi-turn subagents are always enabled**. You can send follow-up messages to a subagent that is still running — useful for steering it mid-task, adding context you forgot to include, or asking it to adjust its approach. Open the `/tasks` panel to see running subagents and their timelines, then type into the subagent's input to send a steering message.
