...
*(v1.0.79+)* Set `"autoUpdate": true` on an `extraKnownMarketplaces` entry to have its plugins automatically update to the latest version at session start, without running `copilot plugin update` manually:
...
> **Note**: First-party plugins (those from the built-in `copilot-plugins` marketplace) already auto-update at session start as of v1.0.78. The `autoUpdate` flag extends this behavior to custom marketplace entries.
