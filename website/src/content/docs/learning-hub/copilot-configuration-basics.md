...
The `/worktree` command (v1.0.61+) creates a new git worktree and switches into it, **leaving your uncommitted changes behind** in the original worktree. The companion `/move` command (v1.0.71+) does the same but **carries your uncommitted changes** into the new worktree. Choose `/worktree` when you want a clean slate and `/move` when you want to continue your current work-in-progress on a new branch:

```
/worktree my-feature-branch   # new worktree; uncommitted changes stay in the current worktree
/move my-feature-branch       # new worktree; uncommitted changes move with you
```

> **Breaking change (v1.0.71)**: Prior to v1.0.71, `/worktree` and `/move` were aliases of each other — both moved uncommitted changes into the new worktree. In v1.0.71 they were split: `/worktree` now leaves changes behind, and `/move` carries them. Update any scripts or workflows that relied on `/worktree` moving uncommitted changes.
...
/move continue the auth refactor on a new branch
```
...
> **Plan mode safety (v1.0.71+)**: Plan mode now **hard-blocks** any built-in tool calls that would modify the workspace — the agent cannot edit files or run mutating shell commands while planning. Built-in mutators (such as opening a pull request) are also blocked; MCP and external tools remain allowed. This makes plan mode a safe way to review what an agent intends to do before authorizing it to act.
