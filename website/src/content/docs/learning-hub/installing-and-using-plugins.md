...
**First-party plugins auto-update**: Plugins from official first-party marketplaces automatically update to the latest version at the start of each Copilot session, so you always get improvements and fixes without running `copilot plugin update` manually.
