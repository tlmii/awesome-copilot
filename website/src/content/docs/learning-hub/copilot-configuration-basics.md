...
The `/worktree` command (v1.0.61+) creates a new git worktree and switches into it. In v1.0.71+, **`/worktree` and `/move` are separate commands** with distinct behaviors:

- **`/worktree`** — creates a new worktree and **leaves your uncommitted changes behind** in the current worktree. Use this when you want a clean slate in the new worktree while preserving in-progress work where it is.
...
> **Note**: Prior to v1.0.71, `/move` was an alias for `/worktree` and both carried uncommitted changes. If you rely on the old behavior (carrying changes), use `/move` going forward.
...
The `/voice devices` command *(v1.0.71+)* lets you choose and persist the microphone used for voice input mode. Run it to open the device picker and select your preferred input device:

```
/voice devices
```

The selected device is persisted across sessions so you do not need to reconfigure it each time you use voice mode. This is useful when you have multiple audio inputs (e.g., a USB headset, a built-in microphone, and a video call device) and want to ensure the CLI always uses the correct one.
...
> **Plan mode hard-block (v1.0.71+)**: Plan mode now **hard-blocks** built-in tool calls that would modify the workspace. While in plan mode, the agent can no longer edit files or run mutating shell commands — built-in mutators such as opening a pull request are blocked. MCP and external tools are still allowed. This makes plan mode a reliable "read-only proposal" mode: you can safely ask for a plan without risking unintended changes.
