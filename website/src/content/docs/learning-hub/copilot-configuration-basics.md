...
|-------|-------------|----------------|
| `model` | The AI model to use for this repository | `"claude-sonnet-4"`, `"gpt-4.1"`, `"gemini-3.7-flash"` |
...
| `defaultMode` | Default interaction mode for new interactive sessions: `interactive`, `plan`, or `autopilot` (v1.0.81+) |
| `defaultPermissionMode` | Default approval behavior for new interactive sessions: `auto`, `manual`, or `allow-all` (v1.0.81+) |
...
**Voice dictation** (v1.0.81+): Press **Ctrl+Space** to toggle voice dictation on and off. This lets you dictate prompts verbally instead of typing them — useful for longer, more conversational instructions.
