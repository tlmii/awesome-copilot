...
In v1.0.71+, the **default maximum subagent nesting depth was lowered from 6 to 4** to curb runaway recursive delegation. Usage-based billing users can still adjust `subagents.maxDepth` (up to 128) from `/settings` if deeper nesting is needed for your workflow.
