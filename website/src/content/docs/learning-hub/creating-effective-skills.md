...
As of v1.0.72, you can also install skills via the plugins command using `copilot plugins install --skill`:
...
You can also run `/skill` (or the existing `/skills`) inside an interactive session to see what's loaded. Both `copilot skill add` and `copilot plugins install --skill` are supported ways to install skills that aren't packaged inside a plugin.
